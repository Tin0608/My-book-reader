...
def get_base64(file_obj):
    if file_obj:
        file_bytes = file_obj.read(); file_obj.seek(0)
...
if book_file:
    ...
    if page_num > 10 and not st.session_state.is_paid:
        ...
        with c1: st.markdown(f'<div class="korean-text">{text}</div>', unsafe_allow_html=True)
        with c2:
            if st.button("🌐 翻译"):
                st.markdown(f'<div class="chinese-text">{translate_text(text)}</div>', unsafe_allow_html=True)
else:
    st.info("👋 上传 PDF 开始沉浸式阅读（前10页试读）")
